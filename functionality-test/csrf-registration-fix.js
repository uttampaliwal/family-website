...
async function testRegistration() {
  try {
    ...
    // Registration data - using fields that match the validation schema
    const registrationData = {
      name: 'Test User',
      email: 'test' + Date.now() + '@example.com', // Unique email to avoid conflicts
      password: 'TestPassword123!',
      dob: '1990-01-01', // Use dob as expected by the validation schema
      mobileNumber: '1234567890', // Use mobileNumber as expected by the validation schema
      username: 'testuser' + Date.now(), // Unique username
      gender: 'Male' // Use 'Male' as expected by the validation schema
    };
