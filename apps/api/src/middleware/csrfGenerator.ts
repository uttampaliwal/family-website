...
const generateCsrfToken = (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction): void => {
...
const validateCsrfToken = (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction): void => {
  // Log when this middleware is hit
  console.log('[CSRF] validateCsrfToken middleware running.', { method: req.method, url: req.url });
  ...
  if (!clientToken || !cookieToken || typeof clientToken !== 'string' || typeof cookieToken !== 'string') {
    console.warn('[CSRF] Validation failed: Token missing or has invalid type.');
