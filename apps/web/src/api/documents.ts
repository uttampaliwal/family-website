...
export const updateDocument = async (
  ...
  const validatedId = validateId(id);
  const response = await api.put(API_ENDPOINTS.DOCUMENT_BY_ID(validatedId), {
    title,
    content,
  });
  return response.data;
...
export const downloadFile = async (id: string): Promise<void> => {
  const validatedId = validateId(id);
  window.open(
    `${api.defaults.baseURL}${API_ENDPOINTS.DOCUMENT_DOWNLOAD(validatedId)}`,
    "_blank",
  );
};
...
export const shareDocument = async (
  ...
  await api.post(API_ENDPOINTS.DOCUMENT_SHARE(validatedId), {
    username: String(validatedUsername),
  });
};
